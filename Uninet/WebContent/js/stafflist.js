...
function handlePageBtnClick(currentPage, method, option, word) {
	...
	$(".prev-page-bundle").click(
			function() {
				if (currentPage % 3 === 0) {
					callData(currentPage - 3, method, option, word).done(
							makePageBtns());
				} else if (currentPage % 3 === 2) {
					callData(currentPage - 2, method, option, word).done(
							makePageBtns());
				} else {
					callData(currentPage - 1, method, option, word).done(
							makePageBtns());
				}
			});
}
...
$(function() {
	...
	$(document).on(
			...
			function() {
				...
				var result = confirm("회원를 정말 삭제하시겠습니까?");

				if (result) {
					location.replace("delete?id="
							+ $(this).siblings("#deleteId").val());
				} else {
